...
from utils import interpolate_cheb
...
print("Test Barycentric formula of Lagrange polynomial using {} Chebyshev nodes.".format(NUM_CHEB_NODES))
...
for id_num in [68]:
    ...
    Seebeck_barycentric_func, Seebeck_exact_func, Seebeck_cheb_nodes = interpolate_barycentric_cheb(
        Seebeck_xy_data[:, 0], Seebeck_xy_data[:, 1], NUM_CHEB_NODES, spline_order=SEEBECK_SPLINE_ORDER
    )
    elec_resi_barycentric_func, elec_resi_exact_func, elec_resi_cheb_nodes = interpolate_barycentric_cheb(
        elec_resi_xy_data[:, 0], elec_resi_xy_data[:, 1], NUM_CHEB_NODES, spline_order=ELEC_RESI_SPLINE_ORDER
    )
    thrm_resi_barycentric_func, thrm_resi_exact_func, thrm_resi_cheb_nodes = interpolate_barycentric_cheb(
        thrm_resi_xy_data[:, 0], thrm_resi_xy_data[:, 1], NUM_CHEB_NODES, spline_order=THRM_RESI_SPLINE_ORDER
    )

    Seebeck_lagrange_func, _, _ = interpolate_cheb(
        Seebeck_xy_data[:, 0], Seebeck_xy_data[:, 1], NUM_CHEB_NODES, spline_order=SEEBECK_SPLINE_ORDER
    )
    elec_resi_lagrange_func, _, _ = interpolate_cheb(
        elec_resi_xy_data[:, 0], elec_resi_xy_data[:, 1], NUM_CHEB_NODES, spline_order=ELEC_RESI_SPLINE_ORDER
    )
    thrm_resi_lagrange_func, _, _ = interpolate_cheb(
        thrm_resi_xy_data[:, 0], thrm_resi_xy_data[:, 1], NUM_CHEB_NODES, spline_order=THRM_RESI_SPLINE_ORDER
    )
